...
mock_methods_grammar = (
    ...
    DOT +
    Keyword("args") + LPAREN + SkipTo(RPAREN)("arguments") + RPAREN +
    Optional(Combine(DOT + SkipTo(SEMICOLON)("verify"))) + SEMICOLON
)

@tokens_as_dict(assert_keys_in=["what", "methodName", "arguments", "verify"])
def mock_methods_replace(tokens):
    ...
    return tokens["what"] + "." + tokens["methodName"] + "(" + tokens["arguments"] + ")" + verify + ";"
